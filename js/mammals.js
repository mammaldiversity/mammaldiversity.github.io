...
MDD.classification = {
  ...
  subgenusList: {},
  specificEpithetList: {},
}; // new format with subclass, infraclass, magnorder, superrorder (replacing majorType and majorSubtype)
...
function createMSW3OrderList(event) {
  Papa.parse(MSW3file, {
    ...
    complete: function (results) {
      ...
      if (results.data[results.data.length - 1].ID == "") {
        // incomplete row at end of file due to extra line feed (first column ID)
      ...
      var output =
        '<ul><li id="' +
        taxonID +
        '"><a href="https://www.departments.bucknell.edu/biology/resources/msw3/browse.asp"><b>Mammalia</b></a> (MSW3)</li></ul>';
...
function createMDDOrderList(event) {
  Papa.parse(MDDfile, {
    ...
    complete: function (results) {
      ...
      var numberRows = results.data.length;
      if (results.data[results.data.length - 1].sciName == "") {
        // incomplete row at end of file due to extra line feed (first column sciName)
        numberRows = results.data.length - 1;
      ...
      var output =
        '<ul><li id="' +
        taxonID +
        '"><a href="https://www.mammaldiversity.org/taxa.html"><b>Mammalia</b></a> (MDD)</li></ul>'; // REMEMBER remove  the url
...
function sortMDDbyTaxonRank() {
  MDD.data.sort((a, b) =>
    a.subclass < b.subclass
      ? 1
      : b.subclass < a.subclass
      ? -1 // reverse alphbetical order
      : a.infraclass > b.infraclass
      ? 1
      : b.infraclass > a.infraclass
      ? -1
      : a.magnorder > b.magnorder
      ? 1
      : b.magnorder > a.magnorder
      ? -1
      : a.superorder > b.superorder
      ? 1
      : b.superorder > a.superorder
      ? -1
      : a.order > b.order
      ? 1
      : b.order > a.order
      ...
      ? 1
      : b.suborder > a.suborder
      ? -1
      : a.infraorder > b.infraorder
      ? 1
      : b.infraorder > a.infraorder
      ? -1
      : a.parvorder > b.parvorder
      ? 1
      : b.parvorder > a.parvorder
      ? -1
      : a.superfamily > b.superfamily
      ? 1
      : b.superfamily > a.superfamily
      ? -1
      : a.family > b.family
      ? 1
      : b.family > a.family
      ? -1
      : a.subfamily > b.subfamily
      ? 1
      : b.subfamily > a.subfamily
      ? -1
      : a.tribe > b.tribe
      ? 1
      : b.tribe > a.tribe
      ...
      : b.genus > a.genus
      ? -1
      : a.subgenus > b.subgenus
      ? 1
      : b.subgenus > a.subgenus
      ? -1
      : a.specificEpithet > b.specificEpithet
      ? 1
      : -1
...
function initialiseTaxonTreeStateMDD() {
  ...
  if (show && show != "") {
    switch (show) {
      case "order":
        expandTaxonTreeMDD(8);
        break;
      case "family":
        expandTaxonTreeMDD(6);
        break;
      case "genus":
        expandTaxonTreeMDD(7);
        break;
    }
    expanded = true;
  }
...
function expandTaxonTreeMDD(mode) {
  switch (mode) {
    ...
    case 4: // expands one level at a time all the way down (very slow; seems to work but ends in error)
      if ($('[id$="-load"]').length == 0) {
        // issue with Craseonycteris thonglongyai (Kitti’s Hog-nosed Bat;
      ...
      $('[id$="-load"]')
        ...
        .ready(function () {
          setTimeout(function () {
            expandTaxonTreeMDD(4);
          }, 5000); //timer for each iteration of the recursion
        });
      break;
    case 5: // expands down to family (first version)
      ...
      $('[id$="-load"]')
        .not(".family")
        .each(function () {
          // select all unloaded elements that are not family
          $(this)
            .trigger("click")
            .ready(function () {
              setTimeout(function () {
                expandTaxonTreeMDD(5);
              }, 500); // timer for each each(); the timer only works first iteration, then seems to runs through the recursions (asynchrony?)
            });
        });
      break;
    case 6: // expands down to family (slightly simpler version with timer, which acts on each step (about 6-7 expansions max)
      ...
      $('[id$="-load"]')
        .not(".family")
        .trigger("click") // select all unloaded elements that are not family
        .ready(function () {
          setTimeout(function () {
            expandTaxonTreeMDD(6);
          }, 500); //timer for each iteration of the recursion
        });
      break;
    case 8: // expands down to order(same method as 6)
      ...
      $('[id$="-load"]')
        .not(".order")
        .trigger("click") // select all unloaded elements that are not order
        .ready(function () {
          setTimeout(function () {
            expandTaxonTreeMDD(8);
          }, 500); //timer for each iteration of the recursion
    ...
    case 8:
      ...
      $("#MDD_" + order + "-load")
        .trigger("click") // load children of order (default CARNIVORA)
        //.ready(function(){   $('#MDD_FELIFORMIA-load, #MDD_CANIFORMIA-load').trigger("click")         // load FELIFORMIA and CANIFORMIA children
        .ready(function () {
          ...
          $(
            "#MDD_CARNIVORA-load, #MDD_FELIFORMIA-load, #MDD_FELOIDEA-load, #MDD_FELIDAE-load, #MDD_FELINAE-load"
          ).trigger("click");
        });
      break;
  }
...
function loadTaxonomyMDD(taxon, rank) {
  ...
  for (i = 0; i < MDD.data.length; i++) {
    //iterate through the rows

    if (taxon == MDD.data[i][rank]) {
      // this is the row
      ...
      if (MDD.data[i]["subclass"] != "NA")
        loadTaxonMDD(MDD.data[i]["infraclass"]);
      if (MDD.data[i]["infraclass"] != "NA")
        loadTaxonMDD(MDD.data[i]["magnorder"]);
      if (MDD.data[i]["magnorder"] != "NA")
        loadTaxonMDD(MDD.data[i]["superorder"]);
      if (MDD.data[i]["superorder"] != "NA") loadTaxonMDD(MDD.data[i]["order"]);

      if (MDD.data[i]["order"] != "NA") loadTaxonMDD(MDD.data[i]["order"]);
      if (MDD.data[i]["suborder"] != "NA")
        loadTaxonMDD(MDD.data[i]["suborder"]);
      if (MDD.data[i]["infraorder"] != "NA")
        loadTaxonMDD(MDD.data[i]["infraorder"]);
      if (MDD.data[i]["parvorder"] != "NA")
        loadTaxonMDD(MDD.data[i]["parvorder"]);
      if (MDD.data[i]["superfamily"] != "NA")
        loadTaxonMDD(MDD.data[i]["superfamily"]);
      if (MDD.data[i]["family"] != "NA") loadTaxonMDD(MDD.data[i]["family"]);
      if (MDD.data[i]["subfamily"] != "NA")
        loadTaxonMDD(MDD.data[i]["subfamily"]);
...
function onChangeOrderList(event, db) {
  ...
  if (event.target.getAttribute("id") == "check-MDD") {
    if (event.target.checked) createMDDOrderList();
    else $("#content-MDD").html("");
  }
  if (event.target.getAttribute("id") == "check-MSW3") {
    if (event.target.checked) {
      ...
      $("#comment")
        .css("clear", "both")
        .html("Comparing MDD (left) and MSW3 (right)");
    } else $("#content-MSW3").html("");
  }
  if (event.target.getAttribute("id") == "check-details") {
    if (event.target.checked) $("#content-details").css("display", "block");
    else $("#content-details").css("display", "none");
  }
}
...
function getParentRankMSW3(taxonID, rank) {
  //not needed
  switch (rank) {
    ...
    case "genus":
      ...
      break;
    case "species":
      parent = MSW3.classification.genera;
      break;
    case "subspecies":
      parent = MSW3.classification.species;
      break;
  }
  return parent;
}
...
function getChildrenMSW3(taxonID, rank, parentRank, db) {
  ...
  var i = 0;
  for (i = parent[taxonID][1]; i < parent[taxonID][2]; i++) {
    //iterate through the rows
    ...
    if (rowRank == rank) {
      //if current row has the required rank (i.e. we have the next taxon of that rank)

      if (currentTaxon) {
        // finish dealing with previous order. Could just use if i != parent[id][1]
      ...
      if (rank == "species")
        currentTaxon =
          "MSW3_" + MSW3.data[i]["Genus"] + "_" + MSW3.data[i]["Species"]; // use binomial for taxonID (species epithets are not unique, e.g. jubatus)
      if (rank == "subspecies")
        currentTaxon =
          "MSW3_" +
          MSW3.data[i]["Genus"] +
          ...
          MSW3.data[i]["Species"] +
          "_" +
          MSW3.data[i]["Subspecies"]; // use trinomial for taxonID
      if (rank == "subgenus")
        currentTaxon =
          "MSW3_" + MSW3.data[i]["Genus"] + "_" + MSW3.data[i]["Subgenus"]; //+ ")";//  to disambiguate genus and nominate subgenera (cannot use (...) in id)

      switch (rank) {
        case "subspecies":
          scientificName = MSW3.data[i]["Species"] + " " + scientificName;
        case "species":
          scientificName = MSW3.data[i]["Genus"] + " " + scientificName; //make species combination
      ...
      switch (rank) {
        ...
        case "species":
        case "subspecies":
          scientificName = "<i>" + scientificName + "</i>"; //add italics for genus, species, subspecies
      }
      ...
      switch (rank) {
        ...
        default:
          scientificName =
            '<a href="https://www.departments.bucknell.edu/biology/resources/msw3/browse.asp?id=' +
            MSW3.data[i]["ID"] +
            '" >' +
            scientificName +
            "</a>";
      ...
      if (rank == "species" || rank == "subspecies") {
        // only use for species and subspecies; at least one genus with extant species is marked extinct
        if (MSW3.data[i]["Extinct?"] == "TRUE") extinct = "†";
      }
      if (MSW3.data[i].CommonName)
        extras += " (" + MSW3.data[i].CommonName + ")";

      // handle synonyms
      if (rank == "species") {
        if (findGetParameter("synonyms") && MSW3.data[i].Synonyms != "") {
          if (
            MSW3.data[i + 1].TaxonLevel != "SUBSPECIES" ||
            findGetParameter("synonyms-all")
          ) {
      ...
      if (rank == "subspecies") {
        if (
          findGetParameter("synonyms") &&
          synonyms[i - parent[taxonID][1] - 1] != ""
        )
      ...
      switch (rank) {
        ...
        case "species":
          if (
            ...
            // if no subspecies greyed out inactive bullet
            output =
              '<li class="' +
              rank +
              regionClasses +
              '"><span class="no_spp species" id="' +
              currentTaxon +
              '">' +
      ...
      if (MSW3.data[i + 1] && MSW3.data[i + 1].TaxonLevel)
        // catch end of file (no need to set value of childRank then
...
function parseSynonymsMSW3(data, speciesRow, subspeciesRow2) {
  ...
  while (
    data[subspeciesRow] &&
    data[subspeciesRow]["TaxonLevel"] == "SUBSPECIES"
  ) {
    ...
    if (subspeciesIndex == 0) {
      // if the nominate subspecies
      ...
      regex = new RegExp(
        "(" +
          ...
          nextSubspecies,
        ""
      ); // regex to capture text between subspecies in synonym list
    }
    ...
    if (result == null || !result[1]) {
      ...
      if (subspeciesIndex == 0)
        parsedResult =
          ...
          currentDate +
          "; " +
          parsedResult; //prepend nominate subspecies name with authority
...
function italiciseName(parsedResult, start, sep) {
  ...
  var j = 0;
  for (j = start; j < namesArray.length; j++) {
    // note j=0 shows the subspecies name in the synonym list, i=1 skips the subspecies and just shows synonyms
    if (namesArray[j] != "" || namesArray[j] != " ") {
      formattedName = namesArray[j]
        .trim()
        .replace(/([a-z]*)\s(.*)/, "<i>$1</i> $2"); //italicise the subspecies names (first word before authority)
...
function getChildrenMDD(taxonID, rank, parentRank, db) {
  ...
  for (i = parent[taxonID][1]; i < parent[taxonID][2]; i++) {
    ...
    if (newTaxon == "MDD_INCERTAE SEDIS" || newTaxon == "MDD_incertae sedis")
      newTaxon = newTaxon.replace(" ", "_") + taxonID.replace("MDD_", "_"); // incertae sedis needs qualifying with parent
    ...
    if (newTaxon != currentTaxon) {
      //if current row has the new value for the required rank (i.e. we have the next taxon of that rank)
      ...
      if (rank == "specificEpithet") {
        ...
        if (MDD.data[i].mainCommonName) {
          ...
          if (MDD.data[i].otherCommonNames)
            extras = extras.replace(
              /\)$/,
              "; " + MDD.data[i].otherCommonNames.replace(/\|/g, ", ") + ")"
            );
        }
        // TODO? nominalNames	taxonomyNotes	taxonomyNotesCitation	biogeographicRealm	countryDistribution	iucnStatus	extinct	domestic

        if (findGetParameter("compare")) {
          // CMW_sciName	diffSinceCMW	MSW3_matchtype	MSW3_sciName	diffSinceMSW3
          if (MDD.data[i].CMW_sciName != MDD.data[i].sciName)
            extras +=
              " [CMW name: " + MDD.data[i].CMW_sciName.replace("_", " ") + "]";
        }
      }
      ...
      switch (rank) {
        ...
        case "specificEpithet":
          scientificName = MDD.data[i]["genus"] + " " + scientificName; //make species combination
          break;
        case "order":
          break;
        default:
          scientificName = firstToUpper(scientificName);
      }
      if (scientificName == "Incertae sedis")
        scientificName = "<i>" + scientificName + "</i>"; //add italics for incertae sedis
      switch (rank) {
        case "genus":
        case "subgenus":
        case "specificEpithet":
          scientificName = "<i>" + scientificName + "</i>"; //add italics for genus, subgenus, species, subspecies
      }
      //alert('"'+scientificName+'"');
      switch (rank) {
        case "specificEpithet":
          scientificName =
            '<a href="https://www.mammaldiversity.org/explore.html#' +
            "genus=" +
            MDD.data[i]["genus"] +
            "&species=" +
            MDD.data[i]["specificEpithet"] +
            "&id=" +
            MDD.data[i]["id"] +
            '" target = "_self" >' +
            scientificName +
        ...
        case "genus":
          scientificName =
            '<a href="https://www.mammaldiversity.org/taxa.html#' +
            rank +
            "=" +
            MDD.data[i][rank] +
            '" target = "_self" >' +
      ...
      switch (rank) {
        case "specificEpithet": // no subspecies in MDD; use greyed out inactive bullet
          output =
            ...
            '<span style="color:#008800;font-weight:500;">&#9432; </span>' +
            extinct +
            scientificName +
            extras +
            "</span></li>"; // info icon &#128712; &#9432;
      ...
      switch (rank) {
        ...
        case "family":
          ...
          break;
        case "genus":
          childRank = "specificEpithet";
          break;
        case "species":
          childRank = "subspecies";
          break;
      }
...
function addLinks(children, rank) {
  $(".linkIOC").each(function () {
    ...
    $(this).attr("href", url).removeClass("linkIOC"); // remove class now link added
  });
}
...
function addTreeInteractivity(taxon, rank, datatype, db) {
  var parentRank = datatype; // used in mammals, birds; is this general
  var expandSymbol = "&#9654;";
  var collapseSymbol = "&#9660;";
  var loadSymbol = "&#9654;"; // black up/right/down/left arrows &#9654; &#9654; &#9660; &#9664; (can be coloured)
  ...
  $(".new-li")
    ...
    .prepend(function () {
      ...
      var spans =
        ...
        '")>' +
        loadSymbol +
        " </span>" + //  +square &#8862;    blue right arrow &#9654;
        '<span id="' +
        id +
        '-collapse" style="display:none;">' +
        collapseSymbol +
        " </span>" + //  -square &#8863;    blue down arrow &#128317;
        '<span id="' +
        id +
  ...
  $(".new-buttons")
    .removeClass("new-buttons")
    .addClass("new_functions")
    .each(function () {
      var id = $(this).attr("id");
